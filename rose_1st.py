...
GRAMMAR_DB = {
    ...
    "어떡해": {"correct": "어떻해", "msg": "받침 'ㅎ'은 '어떻게'에만 써! 👑"},
...
st.markdown("""
    ...
    .rose-container {
        ...
        width: 100%;
        animation: float 4s ease-in-out infinite; /* 둥둥 효과 적용 */
        margin-bottom: 20px;
    }

    /* Streamlit의 이미지 위젯 강제 중앙화 */
    [data-testid="stImage"] {
        display: flex;
        justify-content: center;
    }

    .stButton>button { border-radius: 20px; width: 280px; font-weight: bold; margin: 10px auto; display: block; }
    </style>
...
if st.session_state.page == "profile":
    ...
    with st.form("profile_form", clear_on_submit=True):
        name = st.text_input("이름", placeholder="예: 메이킴")
        mbti = st.selectbox("MBTI", ["선택해줘", "INTJ", "ENFP", "INFJ", "ENTP", "기타"])
        hobby = st.text_input("취미", placeholder="예: 코딩")
        goal = st.text_input("학습 목적", placeholder="예: 한국어 마스터")
...
# --- 8. 채팅 페이지 ---
    ...
    if st.session_state.get("is_thinking"):
        ...
        if grammar:
            response = f"{user['name']}님! 방금 '{grammar['wrong']}'라고 썼지? 그건 '{grammar['correct']}'가 맞아! {grammar['msg']}"
            st.session_state.rose_state = "rose_think.png"
        else:
            if st.session_state.mode == "반말":
                response = f"{user['name']}아! {user['hobby']} 좋아하는구나? '{current_input}' 생각도 아주 갓벽해! 🌹"
                st.session_state.rose_state = "rose_happy.png"
            else:
                response = f"{user['name']}님, {user['goal']}을(를) 위해 노력하시는 모습이 멋져요. '{current_input}'에 대해 정중히 응원할게요! 👑"
